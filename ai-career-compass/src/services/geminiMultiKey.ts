...
const API_KEYS = {
  ...
  flashcards: import.meta.env.VITE_GEMINI_API_KEY_FLASHCARDS || '',
  quiz: import.meta.env.VITE_GEMINI_API_KEY_QUIZ || '',
  studyPlans: import.meta.env.VITE_GEMINI_API_KEY_STUDY_PLANS || '',
  careerAnalysis: import.meta.env.VITE_GEMINI_API_KEY_CAREER_ANALYSIS || '',
  summarization: import.meta.env.VITE_GEMINI_API_KEY_SUMMARIZATION || '',
  practice: import.meta.env.VITE_GEMINI_API_KEY_PRACTICE || '',
  analytics: import.meta.env.VITE_GEMINI_API_KEY_ANALYTICS || '',
};
...
const validateAPIKeys = () => {
  ...
  Object.entries(API_KEYS).forEach(([name, key]) => {
    if (!key || key === `your_gemini_api_key_for_${name}_here`) {
      missingKeys.push(name);
    }
  });
  
  if (missingKeys.length > 0) {
    console.warn(`⚠️ Missing or invalid Gemini API keys for: ${missingKeys.join(', ')}`);
    console.warn('Some AI features may not work. Please add keys to your .env file.');
  }
};

validateAPIKeys();

// Create separate AI instances for each API key
const aiInstances = {
  chatbot: new GoogleGenerativeAI(API_KEYS.chatbot),
  flashcards: new GoogleGenerativeAI(API_KEYS.flashcards),
  quiz: new GoogleGenerativeAI(API_KEYS.quiz),
  studyPlans: new GoogleGenerativeAI(API_KEYS.studyPlans),
  careerAnalysis: new GoogleGenerativeAI(API_KEYS.careerAnalysis),
  summarization: new GoogleGenerativeAI(API_KEYS.summarization),
  practice: new GoogleGenerativeAI(API_KEYS.practice),
  analytics: new GoogleGenerativeAI(API_KEYS.analytics),
};
...
// Multi-Key Gemini Service Class
export class GeminiMultiKeyService {
  ...
  // ============================================
  // FLASHCARD GENERATION (Uses flashcards API key)
  // ============================================
  async generateFlashcards(topic: string, count: number = 10): Promise<Flashcard[]> {
    const model = this.getModel('flashcards');
...
Return ONLY the JSON array, no additional text.`;
  ...
  // ============================================
  // QUIZ GENERATION (Uses quiz API key)
  // ============================================
  async generateQuiz(
    topic: string, 
    ...
    const model = this.getModel('quiz');
...
Return ONLY the JSON array, no additional text.`;
  ...
  // ============================================
  // STUDY PLAN GENERATION (Uses studyPlans API key)
  // ============================================
  async generateStudyPlan(params: {
    ...
    const model = this.getModel('studyPlans');
...
Return ONLY the JSON object, no additional text.`;
  ...
  // ============================================
  // CAREER ANALYSIS (Uses careerAnalysis API key)
  // ============================================
  async analyzeCareerMatch(userProfile: {
    interests: string[];
    skills: string[];
    education: string;
    experience: string;
  }): Promise<any> {
    const model = this.getModel('careerAnalysis');
    
    const prompt = `Analyze career matches for this profile:
...
Provide top 5 career recommendations with match scores, required skills, and growth outlook.
Format as JSON array.`;
  ...
  // ============================================
  // WEAK AREA ANALYSIS (Uses analytics API key)
  // ============================================
  async analyzeWeakAreas(performanceData: {
    ...
    const model = this.getModel('analytics');
...
Return ONLY the JSON array, no additional text.`;
  ...
  // ============================================
  // PRACTICE PROBLEMS (Uses practice API key)
  // ============================================
  async generatePracticeProblems(
    ...
    const model = this.getModel('practice');
...
Return ONLY the JSON array, no additional text.`;
  ...
  // ============================================
  // STUDY TIPS (Uses studyPlans API key)
  // ============================================
...
Return ONLY the JSON array, no additional text.`;
  ...
  // ============================================
  // PROGRESS ANALYSIS (Uses analytics API key)
  // ============================================
...
Return ONLY the JSON object, no additional text.`;
  ...
  // ============================================
  // CONTENT SUMMARIZATION (Uses summarization API key)
  // ============================================
  async summarizeContent(content: string, maxLength: number = 500): Promise<{
    ...
    const model = this.getModel('summarization');
...
Return ONLY the JSON object, no additional text.`;
  ...
  // ============================================
  // CHATBOT (Uses chatbot API key)
  // ============================================
  async chat(message: string, context?: string): Promise<string> {
    const model = this.getModel('chatbot');
...
export const geminiMultiKeyService = new GeminiMultiKeyService();

// Export API key status for debugging
export const getAPIKeyStatus = () => {
  return Object.entries(API_KEYS).map(([name, key]) => ({
    name,
    configured: !!(key && key !== `your_gemini_api_key_for_${name}_here`)
  }));
};
